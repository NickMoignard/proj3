...
class OhHellModel():
	...
	def flatten_suits(sorted_cards, trump_suit=None):
	""" name: flatten_suits()
...
class NPC(OhHellModel):
	...
	def make_play(curr_trick, hand, prev_tricks, player_no, deck_top, phase_bids,
...
class Phase(OhHellModel):
	...
	def score_phase(
		bids, tricks, deck_top, player_data=None,
		suppress_player_data=True, self
	):
...
class Game(OhHellModel):
	...
	_score = ("4 tuple")
	_cur_phase = int()
	_dealer = itertools.cycle([0, 1, 2, 3])
	_lead = itertools.cycle([0, 1, 2, 3])
	_players = ["player objects"]
	...
	def __init__(self):
		next(_dealer);
		next(_lead); next(_lead)  # lead player is one to the left of dealer
